...
size = random.randint(900,1000)#星球尺寸The size of planet
...
light_0 = sphere(2,Image.new(mode="RGBA",size=(4096,4096),color=(0,0,0,0)),1808,canvassize=4096).filter(ImageFilter.BoxBlur(150))
...
Project_starlight = Project_starlight.resize((int(200), int(200)))
if not ishx:
...
Project_image.paste(Project_hxend.filter(ImageFilter.GaussianBlur(80)),(int(2.56 * ((1024-size)**2)/l * math.cos(math.radians(shadow_direction + 180))),int(-2.56 * ((1024-size)**2)/l * math.sin(math.radians(shadow_direction + 180)))),Project_hxend.filter(ImageFilter.GaussianBlur(80)))#用什么公式才能让恒星相对行星半径和阴影位置合理
Project_image.paste(Project_hxend,(int(2.56 * ((1024-size)**2)/l * math.cos(math.radians(shadow_direction + 180))),int(-2.56 * ((1024-size)**2)/l * math.sin(math.radians(shadow_direction + 180)))),Project_hxend)#用什么公式才能让恒星相对行星半径和阴影位置合理
Project_image.paste(Project_hxend.filter(ImageFilter.GaussianBlur(250)),(int(2.56 * ((1024-size)**2)/l * math.cos(math.radians(shadow_direction + 180))),int(-2.56 * ((1024-size)**2)/l * math.sin(math.radians(shadow_direction + 180)))),Project_hxend.filter(ImageFilter.GaussianBlur(250)))#用什么公式才能让恒星相对行星半径和阴影位置合理
Project_image.paste(Project_starlight,(0,0),Project_starlight)#问：如果这里粘贴的位置为结果的中心，那输入坐标是什么式子
